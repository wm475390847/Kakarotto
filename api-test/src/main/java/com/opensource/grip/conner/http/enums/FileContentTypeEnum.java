...
public enum FileContentTypeEnum {
    ...
    FileContentTypeEnum(String suffix, String desc, String contentType) {
        this.suffix = suffix;
        this.desc = desc;
        this.contentType = contentType;
    }

    public static FileContentTypeEnum findBySuffix(String suffix) {
        if (StringUtils.isBlank(suffix)) {
            return null;
        }
        for (FileContentTypeEnum typeEnum : FileContentTypeEnum.values()) {
            if (suffix.equals(typeEnum.getSuffix())) {
                return typeEnum;
            }
        }
        return FileContentTypeEnum.BIN;
    }
     ...
     * @return content-type
     */
    public static FileContentTypeEnum findByFileName(String fileName) {
        int lastDotIndex = fileName.lastIndexOf('.');
        if (lastDotIndex == -1) {
            return FileContentTypeEnum.BIN;
        }
        String suffix = fileName.substring(lastDotIndex + 1);
        return FileContentTypeEnum.findBySuffix(suffix);
    }
}